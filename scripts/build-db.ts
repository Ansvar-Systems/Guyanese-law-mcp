#!/usr/bin/env tsx
/**
 * Database builder for Guyana Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Guyana Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Guyanese statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'GY');
    insertMeta.run('source', 'guyana.justia.com');
