#!/usr/bin/env tsx
/**
 * Guyana Law MCP -- Census-Driven Ingestion Pipeline
 *
 * Reads data/census.json and fetches + parses every ingestable Act
 * from parliament.gov.gy (PDF downloads).
 *
 * Pipeline per law:
 *   1. Download PDF from parliament.gov.gy/publications/acts-of-parliament/
 ...
 *
 * Data source: parliament.gov.gy (National Assembly of Guyana)
...
import { parseGYLawPdf, type ActIndexEntry, type ParsedAct } from './lib/parser.js';
...
async function main(): Promise<void> {
  ...
  console.log('Guyana Law MCP -- Ingestion Pipeline (Census-Driven)');
  console.log('=====================================================\n');
  console.log('  Source: parliament.gov.gy (National Assembly of Guyana)');
  ...
  for (const law of acts) {
    ...
    try {
      ...
      // Parse PDF
      const parsed = parseGYLawPdf(sourceFile, act);
  ...
  console.log(`\n  Source:      parliament.gov.gy (PDF extraction)`);
