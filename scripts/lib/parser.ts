/**
 * Guyana Law PDF Parser
 *
 * Parses law text extracted from PDFs downloaded from
 * mola.gov.gy. Uses `pdftotext` (via execFileSync) for extraction,
 * then applies regex-based section parsing.
 *
 * Guyana follows English common law (former British Guiana):
 *   "Section 1." / "1." / "1.-(1)"
 *   "PART I", "PART II"
...
// Belize section patterns (English common law, similar to Jamaica/TT)
...
function findLawTextStart(text: string): number {
  const startPatterns = [
    /\bENACTED\s+by\s+the\s+National\s+Assembly\b/i,
...
export function parseGYLawText(text: string, act: ActIndexEntry): ParsedAct {
...
export function parseGYLawPdf(pdfPath: string, act: ActIndexEntry): ParsedAct {
